...
def get_secret_value() -> Optional[str]:
...
def compare(si: Dict[str, Any], bl: Dict[str, Any], confidence: float) -> Dict[str, Any]:
    ...
    return {
        ...
        "review_reason": "; ".join(reasons) or ("AI confidence below 80%" if confidence < 80 else ""),
...
def make_report(email: Dict[str, Any], si: Dict[str, Any], bl: Dict[str, Any], confidence: float, source: str) -> Dict[str, Any]:
    ...
    return {
        "email_id": email.get("id", "upload"),
        "category": "document-comparison request",
        "generated_at": datetime.utcnow().isoformat() + "Z",
        "extraction_source": source,
        "confidence": round(confidence, 1),
        "needs_human_review": comparison["needs_review"],
        "review_reason": comparison["review_reason"],
        "result": comparison["result"],
        "mismatches": comparison["mismatches"],
        "fields": comparison["rows"],
    }
...
st.markdown(
    ...
    .hero {padding: 1.4rem 1.6rem; border: 1px solid #dce5e6; border-radius: 16px; background: linear-gradient(115deg, #f4faf8, #fff);}
    .eyebrow {color: #157f72; font-size: .75rem; font-weight: 700; letter-spacing: .12em; text-transform: uppercase;}
    .muted {color: #667477;}
    </style>
    """,
...
st.markdown(
    '<div class="hero"><div class="eyebrow">InScout · shipment desk</div><h1>Clearer documents. Fewer surprises.</h1><p class="muted">A practical review queue for sorting inbox traffic and checking SI drafts against BL drafts.</p></div>',
    unsafe_allow_html=True,
)
...
with st.sidebar:
    ...
    st.divider()
    gemini_on = bool(get_secret_value() or os.getenv("GEMINI_API_KEY"))
...
if page == "Inbox":
    ...
    selected_id = st.selectbox("Or choose a demo verification request", [e["id"] for e in DEMO_EMAILS if e["category"] == CATEGORIES[0]])
    selected_email = next(e for e in DEMO_EMAILS if e["id"] == selected_id)
    ...
    if st.button("Run verification", type="primary"):
        if si_file and bl_file:
            si, si_conf, si_source = extract_document(file_text(si_file))
            bl, bl_conf, bl_source = extract_document(file_text(bl_file))
            confidence = min(si_conf, bl_conf)
            source = f"{si_source}; {bl_source}"
        else:
            si = selected_email["si"]
            bl = selected_email["bl"]
            confidence = 92.0
    ...
    if st.session_state.reports.get(selected_id):
        report = st.session_state.reports[selected_id]
        st.success(report["result"])
        x, y, z = st.columns(3)
        x.metric("Confidence", f"{report['confidence']:.0f}%")
        y.metric("Mismatches", len(report["mismatches"]))
        z.metric("Decision", "Human review" if report["needs_human_review"] else "Auto-pass")

        if report["needs_human_review"]:
            st.warning(report["review_reason"] or "This comparison needs a human check.")

        st.dataframe(
            report["fields"],
            column_config={"match": st.column_config.CheckboxColumn("Match")},
            hide_index=True,
        ...
        st.download_button(
            ...
            file_name=f"{report['email_id']}-report.json",
